...
if (!class_exists('Installer')):
	...
	class Installer
		...
		public static function activate() {
			...
			if (isset($settings['kanzu_support_version'])) {
//Reactivation or upgrade
		...
		public function upgrade_plugin($previous_version) {
			...
			if ($sanitized_version < 213) {
//@since 2.2.0 Added ksd_notifications
				$this->set_default_notifications();
			}

			if ($sanitized_version < 229) {
//@since 2.2.9 Added admin, supervisor and agent custom roles
				KSD()->roles->create_roles();
				KSD()->roles->modify_all_role_caps('add');
			...
			if (count($dbChanges) > 0) {
				//Make the Db changes. We use $wpdb->query instead of dbDelta because of
