...
class View {
	 ...
	 * NB: Because we use {@link Users_Controller}, call this function after {@link do_admin_includes} has been called.
	 *
	...
	public function populate_ticket_columns( $column_name, $post_id ) {
		...
		if ( $column_name == 'replies' ) {
			...
			$reply_count
			= $wpdb->get_var(
				" SELECT COUNT(ID) FROM {$wpdb->prefix}posts WHERE "
	...
	public function edit_submitdiv() {
		...
		include_once KSD_PLUGIN_DIR . 'templates/admin/metaboxes/html-ticket-info.php';
	}
